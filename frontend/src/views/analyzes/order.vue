<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      ...
      <el-button class="filter-item" type="success" style="float:right" v-waves icon="edit" @click="handleAnalyze">统计</el-button>
    </div>

    <el-table :data="list" v-loading.body="listLoading" border fit highlight-current-row style="width: 100%">
      <el-table-column align="center" label="卖家" width="110px">
        <template scope="scope">
          <span>{{scope.row.seller_name}}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog title="统计订单和采购" size="tiny" :visible.sync="dialogAnalyzeVisible">
      <el-form class="small-space" :model="analyzeData" label-position="left" label-width="70px" style='width: 400px; margin-left:50px;'>
        <el-form-item label="统计月">
	  <el-date-picker v-model="analyzeData.analyze_time" type="datetime" placeholder="选择日期" :picker-options="pickerOptions0">
	  </el-date-picker>
        </el-form-item>
      </el-form>

      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogAnalyzeVisible = false">取 消</el-button>
        <el-button type="primary" @click="analyze">统 计</el-button>
      </div>
    </el-dialog>
...
<script>
  import { fetchOrderAnalyze, analyzeOrderAndPurchase } from 'api/analyzes';
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        fetchOrderAnalyze(this.listQuery).then(response => {
      ...
      analyze() {
        analyzeOrderAndPurchase(this.analyzeData).then(() => {
          this.dialogAnalyzeVisible = false;
          this.$notify({
            title: '成功',
            message: '统计成功',
            type: 'success',
